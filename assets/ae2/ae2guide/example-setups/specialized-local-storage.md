...
[MEインターフェースの特殊な挙動](../items-blocks-machines/interface.md#special-interactions)の1つを利用することで、
...
<BoxAnnotation color="#dddddd" min="2 0 0" max="2.3 1 1">
        (4) MEインターフェース #2：デフォルト設定
  </BoxAnnotation>

<BoxAnnotation color="#dddddd" min="1.7 0 0" max="2 1 1">
        (5) MEストレージバス：メインストレージより高い優先度に設定。ファーム出力に合わせてフィルタ可能
  </BoxAnnotation>
...
* サブネット上の<ItemLink id="interface" />はメインネットワーク側の<ItemLink id="storage_bus" />に対し、<ItemLink id="drive" />内の内容を提示する
  これによりMEストレージバスはドライブ内のセルへ直接アイテムを出し入れできる
* MEストレージバスは高い[優先度](../ae2-mechanics/import-export-storage.md#storage-priority)を持つため、アイテムはメインストレージよりもサブネット側へ優先的に格納される
