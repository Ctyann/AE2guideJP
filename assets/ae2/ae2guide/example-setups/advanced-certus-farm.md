...
これは基本的に[半自動ケルタスファーム](semiauto-certus-farm.md)と同じですが、完全にあなたのMEシステムへ統合された構成です。
...
<GameScene zoom="6" interactive={true}>
  ...
  <BoxAnnotation color="#ddaaaa" min="3.7 2 1" max="4 3 2">
        (1) ME消滅プレーン #1：GUIなし（設定不可）だが幸運を付与可能
  </BoxAnnotation>

  <BoxAnnotation color="#ddaaaa" min="2 2 1.7" max="3 3 2">
        (2) MEストレージバス #1：ケルタスクリスタルにフィルタリング
  ...
  <BoxAnnotation color="#aaddaa" min="3.7 1 1" max="4 2 2">
        (3) ME消滅プレーン #2：GUIなし（設定不可）だがシルクタッチ必須
  </BoxAnnotation>

  <BoxAnnotation color="#aaddaa" min="2 1 1.7" max="3 2 2">
        (4) MEストレージバス #2：ケルタスブロックにフィルタリング
  ...
  <BoxAnnotation color="#ffddaa" min="2 0.7 2" max="3 1 3">
        (6) MEインポートバス：傷ついた芽生えたケルタスクォーツにフィルタリング
  ...
  <BoxAnnotation color="#aaaadd" min="1.7 2 2" max="2 3 3">
        (7) MEストレージバス #3：ケルタスクリスタルにフィルタリング。メインストレージより高優先度
        <ItemImage id="certus_quartz_crystal" scale="2" />
  </BoxAnnotation>

  <BoxAnnotation color="#aaaadd" min="2 1 2" max="3 2 3">
        (8) MEインターフェース：傷ついた芽生えたケルタスクォーツを1つ保持する設定。クラフティングカード付き
...
* 最初の<ItemLink id="annihilation_plane" />（1）はGUIなしで設定不可だが、幸運を付与可能
...
* <ItemLink id="interface" />（8）は傷ついた芽生えたケルタスクォーツを1つ保持するよう設定され、<ItemLink id="crafting_card" />を搭載

## 動作原理

### 水晶破壊：

水晶破壊サブネットは[シンプルなケルタスファーム](simple-certus-farm.md)とほぼ同じ仕組みです。
...
2. <ItemLink id="storage_bus" />がケルタスクリスタルを樽に保存します

### ケルタスブロック破壊：

芽生えたケルタスクォーツが枯渇して通常の<ItemLink id="quartz_block" />になったときに破壊するためのサブネットです。

1. <ItemLink id="annihilation_plane" />は前方を破壊しようとしますが、MEストレージバスが<ItemLink id="quartz_block" />にフィルタされているためそれのみ破壊可能です
   また、シルクタッチが必要で、芽生えたケルタスクォーツの劣化を防ぎます
2. <ItemLink id="storage_bus" />がブロックを<ItemLink id="interface" />へ送り、水中投げ込み自動化で新しい<ItemLink id="flawed_budding_quartz" />を生成します

### 芽生えたケルタスクォーツ配置：

枯渇ブロックが破壊された後、新しい芽生えたケルタスクォーツを設置する役割です。

1. <ItemLink id="import_bus" />がMEインターフェースから芽生えたケルタスクォーツを[ネットワークストレージ](../ae2-mechanics/import-export-storage.md)へ取り込みます
2. サブネット内の唯一のストレージである<ItemLink id="formation_plane" />がそれを設置します

### メインネットワーク側：

* <ItemLink id="storage_bus" />はメインネットワーク（および[チャージャー自動化](charger-automation.md)）がケルタスクリスタルへアクセスできるようにします。高い[優先度](../ae2-mechanics/import-export-storage.md#storage-priority)により、メインストレージより先に樽へ戻されます
* <ItemLink id="interface" />はサブネットに芽生えたケルタスクォーツを供給し、またケルタスブロック破壊から戻ってきたブロックを受け取ります。さらに<ItemLink id="crafting_card" />により、メインネットワークの[自動クラフト](../ae2-mechanics/autocrafting.md)から新しい芽生えたケルタスクォーツを要求できます
