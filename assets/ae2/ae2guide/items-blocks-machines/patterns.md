...
*   <ItemLink id="crafting_pattern" />は作業台で作成されるレシピを記録します。直接<ItemLink id="molecular_assembler" />に入れることで、
    ...
    この場合、MEパターンプロバイダーは特別な動作を行い、隣接する組立機へ材料と一緒に対応するパターンを送信します。
    組立機はクラフト結果を自動的に隣接インベントリへ排出するため、クラフトパターンの自動化に必要なのはMEパターンプロバイダー上の組立機だけです。
...
*   <ItemLink id="processing_pattern" />は、自動クラフトにおける柔軟性の大部分を担うものです。最も汎用的な種類であり、単純に
    「MEパターンプロバイダーがこれらの材料を隣接インベントリへ押し出した場合、MEシステムは近い将来または遠い将来のどこかで
...
「丸石8 = 石8」とパターンを設定することもでき、その場合MEパターンプロバイダーは製錬設備へ丸石を1個ずつではなく
1回の処理で8個まとめて投入します。
